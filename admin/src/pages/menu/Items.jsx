...
import {
    ...
    EditOutlined, SearchOutlined, DeleteOutlined, EyeOutlined,
    PoweroffOutlined, CopyOutlined, InboxOutlined, DollarOutlined,
    AppstoreOutlined, ExclamationCircleOutlined, ClockCircleOutlined,
    FireOutlined, ThunderboltOutlined, PictureOutlined, CalendarOutlined,
    SettingOutlined
...
const mockItems = [
    ...
    { key: '6', id: 6, name: 'Vanilla Ice Cream', image: 'https://i.pravatar.cc/40?img=25', store: 'Sri Sai Ice-Cream & Juices', category: 'Ice Creams', subCategory: 'Scoops', basePrice: 50, discount: 0, finalPrice: 50, stock: 40, isVeg: true, popular: false, rating: 4.1, prepTime: '2 mins', createdAt: '6 months ago', status: true },
];
...
export const Items = () => {
    ...
    const calcFinal = () => {
        ...
        const finalP = discPrice + (discPrice * tax / 100);
        form.setFieldsValue({ discountPrice: Math.round(discPrice), finalPrice: Math.round(finalP) });
    };
    ...
    const handleSave = () => {
        form.validateFields().then(vals => {
            if (editingRecord) {
                ...
                setData(prev => [...prev, { key: String(id), id, ...vals, rating: 0, createdAt: 'Just now', status: true }]);
    ...
    const columns = [
        ...
        { title: 'Image', dataIndex: 'image', key: 'image', width: 65, render: src => <Avatar src={src} shape="square" size={36} /> },
        {
            ...
            render: (name, record) => (
                <Space direction="vertical" size={2}>
                    <Space size={4}>
                        <Text style={{ color: '#1890ff', cursor: 'pointer', fontWeight: 500 }} onClick={() => openEdit(record)}>{name}</Text>
                        {record.popular && <Tag color="error" style={{ fontSize: 10, padding: '0 4px', border: 'none', borderRadius: 3 }}>🔥 Popular</Tag>}
                    </Space>
                    <Tag color={record.isVeg ? '#52c41a' : '#ff4d4f'} style={{ fontSize: 10, borderRadius: 2, margin: 0 }}>{record.isVeg ? '🟢 Veg' : '🔴 Non-Veg'}</Tag>
                </Space>
            )
        },
        ...
        },
        {
            ...
            render: qty => <Tag color={qty === 0 ? '#ff4d4f' : qty < 10 ? 'warning' : 'success'}>{qty === 0 ? 'Out of Stock' : `${qty} left`}</Tag>
        },
            ...
            >
                <Form form={form} layout="vertical" style={{ marginTop: 8 }}>
                    <Tabs defaultActiveKey="1" tabPosition="left" size="small">
                        <TabPane tab={<span><AppstoreOutlined /> Basic Info</span>} key="1">
                            <Row gutter={16}>
                            ...
                            <Row gutter={16}>
                                ...
                                <Col span={8}><Form.Item label="Category" name="category" rules={[{ required: true }]}><Select placeholder="Select Category">{['Biryani', 'Milk Shakes', 'French Fries', 'Ice Creams', 'Curries'].map(c => <Option key={c} value={c}>{c}</Option>)}</Select></Form.Item></Col>
                            ...
                            <Row gutter={16}>
                                ...
                                <Col span={16}><Form.Item label="Tags" name="tags"><Select mode="tags" placeholder="Bestseller, Spicy, Chef's Special..." /></Form.Item></Col>
                            </Row>
                            <Form.Item label="Description" name="description"><Input.TextArea rows={3} /></Form.Item>
                        </TabPane>

                        <TabPane tab={<span><DollarOutlined /> Pricing</span>} key="2">
                            <Row gutter={16}>
                        ...
                        <TabPane tab={<span><SettingOutlined /> Variants</span>} key="3">
                                ...
                                {(fields, { add, remove }) => (<>
                                    {fields.map(({ key, name, ...rest }) => (
                                        <Card key={key} size="small" style={{ marginBottom: 12, background: isDarkMode ? '#1f1f1f' : '#fafafa' }}>
                                            <Row gutter={12}>
                                                <Col span={5}><Form.Item {...rest} name={[name, 'label']} label="Name"><Input placeholder="e.g. Half" /></Form.Item></Col>
                        ...
                        <TabPane tab="Add-ons" key="4">
                            <Form.List name="addons" initialValue={[{}]}>
                                {(fields, { add, remove }) => (<>
                                    {fields.map(({ key, name, ...rest }) => (
                                        <Card key={key} size="small" style={{ marginBottom: 12, background: isDarkMode ? '#1f1f1f' : '#fafafa' }}>
                                            <Row gutter={12}>
                                                <Col span={6}><Form.Item {...rest} name={[name, 'label']} label="Name"><Input placeholder="e.g. Extra Cheese" /></Form.Item></Col>
                                                <Col span={4}><Form.Item {...rest} name={[name, 'price']} label="Price (₹)"><InputNumber min={0} style={{ width: '100%' }} /></Form.Item></Col>
                        ...
                        <TabPane tab={<span><CalendarOutlined /> Schedule</span>} key="6">
                        ...
                        <TabPane tab={<span><PictureOutlined /> Media</span>} key="7">
